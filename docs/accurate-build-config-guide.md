...
接口仓文件变更，若影响SDK生成（如增删接口），则需编译XTS目标验证，使用配置文件来管理接口文件归属的部件、影响的XTS目标
...
`bundle_name`
> 类型：字符串列表，该接口目录或文件路径所归属/影响的部件仓名称，**此项必填，不能为空**

`build_targets`
> 类型：对象，用于指定不同XTS套件中除`bundle_name`中部件对应的XTS目标外，受影响的额外XTS目标
...
配置路径：[`test/xts/tools/config/ci_api_ignore.json`](https://gitcode.com/openharmony/xts_tools/blob/master/config/ci_api_ignore.json)

并非所有接口仓文件或目录变更都影响SDK生成（如文档、许可文件等），因此这部分变更无需触发XTS编译，可在此文件中配置白名单进行过滤。
...
### 配置说明
`<interface_repo_name>`
> 类型：字符串，接口仓名称，与[接口仓变更](#接口仓变更)小节一致
>
> 可为各接口仓配置忽略规则列表，规则类型为字符串

**路径书写规范**
- 路径为相对路径（相对于接口仓），禁止以`/`开头
- 禁止出现`./`或`../`等依赖路径上下文计算的模式
- 可指定到特定文件或目录，也可使用通配符模糊匹配，详见下文

**匹配规则**：
- **单星号（`*`, Asterisk）**：匹配同一目录层级下的任意字符，遇路径分隔符`/`即停止匹配
- **双星号（`**`, Globstar）**：匹配任意字符，可跨越任意层级的路径分隔符`/`进行递归匹配。若将`**`混合在其它非分隔符字符中使用（如`path/**tail/`），将自动降级为`*`，此时`path/**tail`等价于`/path/*tail`
- **目录强类型指示`/`**：规则以`/`结尾时，作为强类型指示，仅能匹配目录（及目录下所有内容）。若路径表示一个文件，则此规则无法匹配
- **保留标识`!`**：规则以`!`开头表示“保留”该匹配项。被保留的文件不会被忽略，必须在[`test/xts/tools/config/ci_api_part_name.json`](https://gitcode.com/openharmony/xts_tools/blob/master/config/ci_api_part_name.json)正确配置归属部件，否则将导致后续编译失败

**匹配优先级**：
- 按配置列表**从上到下**的顺序执行匹配
- **首次匹配生效（First-Match-Wins）**：若文件或其父级目录被某条规则命中，则直接决定该文件是“忽略”还是“保留”，后续规则将不再处理该文件
- **使用技巧**：若想忽略整个目录，但保留该目录下某个特定文件，请先配置文件的保留规则，随后配置目录的忽略规则。例如：
  ```json
  [
    "!docs/KEEPME.md",
    "docs/"
...
`subdirectory`
> 类型：对象，支持以子目录粒度配置变更仓的XTS编译目标，除新增一个`suite_type`配置项外，子目录配置项类型与取值与上述无异
