...
XTS子系统是OpenHarmony生态认证测试套件的集合，当前包括acts（application compatibility test suite）应用兼容性测试套件，后续会拓展dcts（distributed compatibility test suite）分布式兼容性测试套件、hats（hardware abstract test suite) HAL兼容性测试套件。
