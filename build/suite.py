...
class XDeviceBuilder:
    ...
    def build_xdevice(self):
        ...
        if (xtssuite_out_dir.find('/acts/') != -1):
            ...
            iotc_validator_dir = os.path.join(self.args.suite_out_dir, '../iotc-validator')
            iotc_validator_tools_dir = os.path.join(self.args.suite_out_dir, '../iotc-validator/tools')
            utils.copy_file(output=iotc_validator_tools_dir, source_dirs=gen_dir0,
                            to_dir=True)
            utils.copy_file(output=iotc_validator_tools_dir, source_dirs=gen_dir1,
                            to_dir=True)
            utils.copy_file(output=iotc_validator_dir, sources=run_scripts,
                            to_dir=True)
        ...
        if self.args.configs_dir:
            ...
            if (xtssuite_out_dir.find('/acts/') != -1):
                ...
                utils.copy_file(output=acts_validator_config_dir, 
                                source_dirs=self.args.configs_dir, to_dir=True)
                iotc_validator_config_dir = os.path.join(self.args.suite_out_dir, '../iotc-validator/config')
                utils.copy_file(output=iotc_validator_config_dir,
                                source_dirs=self.args.configs_dir, to_dir=True)
