...
class Test(unittest.TestCase):
    ...
    def test_selection_001(self):
            self.LE.init_runner('test_selection_001')  # 打开runner页面
            self.LE.click_js()  # 取消勾选js
            self.LE.click_manual()  # 取消勾选manual
            self.LE.send_path('selection/caret/collapse-pre-linestart-1.html')  # 是否进入指定路径 默认是/ 即全部路径
            ...
            self.LE.runner_end('test_selection_001')

    def test_selection_002(self):
        self.LE.init_runner('test_selection_002')  # 打开runner页面
        self.LE.click_js()  # 取消勾选js
        self.LE.click_manual()  # 取消勾选manual
        self.LE.send_path('selection/caret/collapse-pre-linestart-2.html')  # 是否进入指定路径 默认是/ 即全部路径
        self.LE.start_test()  # 点击start test 按钮
        self.LE.click_show_test()  # 点击show test按钮
        self.LE.test_screenshot('//*[@id="target"]', "test")  # test页面截图
        self.LE.click_show_ref()  # 点击show reference按钮
        self.LE.ref_screenshot('//*[@id="target"]', "ref")  # reference页面截图
        self.LE.test_assert("test", "ref")  # 断言
        self.LE.runner_end('test_selection_002')
