...
class Test(unittest.TestCase):
    ...
    def test_FileAPI_001(self):
        self.LE.init_runner('test_FileAPI_001')  # 打开runner页面
        self.LE.click_js()  # 取消勾选js
        self.LE.click_manual()  # 取消勾选manual
        self.LE.send_path('FileAPI/url/url_xmlhttprequest_img.html')  # 是否进入指定路径 默认是/ 即全部路径
        ...
        self.LE.test_screenshot('//*[@id="fileDisplay"]', "test")  # test页面截图
        self.LE.click_show_ref()  # 点击show reference按钮
        self.LE.ref_screenshot('/html/body/p[1]', "ref")  # reference页面截图
        self.LE.ref_screenshot('/html/body/p[2]/img', "ref")  # reference页面截图
        self.LE.test_assert("test", "ref")  # 断言
        self.LE.runner_end('test_FileAPI_001')
